...
type ReturnTripUnitProps = {
  bls: IBLS | null;
  refresh: () => void;
};

interface IReturnTripUnitForm {
  frontal?: string;
...
const ReturnTripUnit: React.FC<ReturnTripUnitProps> = ({ bls, refresh }) => {
  ...
  const onSubmit = (data: IReturnTripUnitForm) => {
    if (bls) {
      const entity = {
        ...bls,
        ...data,
  ...
  return (
    <GenericModal id="blsReturnTripModal" title="Revisión de regreso a viaje" size='lg'>
