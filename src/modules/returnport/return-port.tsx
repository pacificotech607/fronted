...
import { blsAggregateAssignPort, statusBls } from '../../constants/bls.constans';
import { get } from 'lodash';
import { getEntities } from '../../entities/bls/bls.reducer';
import BlsDetailContainer from '../../entities/bls/bls-detail-container';
import ReturnPortUnit from './return-port-unit';
import ReturnPortPdfReport from './return-port-pdf-report';

const ReturnPort: React.FC = () => {
    ...
    const [searchQuery, setSearchQuery] = useState<string | null>(
        JSON.stringify([
            ...blsAggregateAssignPort,
            { $match: { alive: true, status: statusBls.exitReviewPort.value } },
        ])
    );
    ...
    const handleSearch = (query: string | null) => {
        const baseQuery = blsAggregateAssignPort;
        const status = viewHistory ? statusBls.returnReviewPort.value : statusBls.exitReviewPort.value;
    ...
    const handleToggleView = () => {
        ...
        const status = newViewHistory ? statusBls.returnReviewPort.value : statusBls.exitReviewPort.value;
        const baseQuery = blsAggregateAssignPort;
    ...
    return (
        <div className="app-main">
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-sm-6">
                            <h3 className="mb-0">Regreso a puerto</h3>
                        </div>
                                    ...
                                    <td className='text-center'>
                                        {viewHistory ? (
                                            ...
                                            <button
                                                ...
                                                data-bs-target="#blsReturnPortModal"
                                                onClick={() => setBl(bl)}
                                                title="Revisar regreso"
                                                disabled={Object.values(statusBls).find(s => s.value === bl.status)?.value !== statusBls.exitReviewPort.value}
                                            >
            ...
            <ReturnPortUnit bls={bl} refresh={() => { dispatch(getEntities(0, 20, searchQuery || undefined)); }} />
            {showPdfReport && <ReturnPortPdfReport bl={bl} onClose={() => setShowPdfReport(false)} />}
        </div>
    );
};

export default ReturnPort;
