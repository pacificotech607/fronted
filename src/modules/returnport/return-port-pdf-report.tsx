...
interface ReturnPortPdfReportProps {
    bl: IBLS | null;
    onClose: () => void;
}

const ReturnPortPdfReport: React.FC<ReturnPortPdfReportProps> = ({ bl, onClose }) => {
    const reportRef = useRef<HTMLDivElement>(null);
    const [imageUrls, setImageUrls] = useState<string[]>([]);

    useEffect(() => {
        const photos = bl?.returnPhotos;
        if (photos) {
            const fetchImageUrls = async () => {
                try {
                    ...
                    setImageUrls(urls.filter((url: string | null): url is string => !!url));
                } catch (error) {
    ...
    const generatePdf = () => {
        if (reportRef.current) {
            html2canvas(reportRef.current, { useCORS: true }).then(canvas => {
                ...
                pdf.save(`reporte-regreso-${bl?.bl || 'N/A'}.pdf`);
            });
        }
    };
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <div ref={reportRef} style={{ padding: '20px' }}>
                            <h1 className="text-center mb-4">Reporte de Regreso a Puerto</h1>
...
export default ReturnPortPdfReport;
