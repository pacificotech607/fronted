...
import TripExitReviewUnit from './trip-exit-review-unit';
import TripExitPdfReport from './trip-exit-pdf-report';

const TripExitReview: React.FC = () => {
    ...
    const [searchQuery, setSearchQuery] = useState<string | null>(
        JSON.stringify([
            ...
            { $match: { alive: true, status: statusBls.assignedTrip.value } },
        ])
    );
    ...
    const handleSearch = (query: string | null) => {
        ...
        const status = viewHistory ? statusBls.exitReviewTrip.value : statusBls.assignedTrip.value;
    ...
    const handleToggleView = () => {
        ...
        const status = newViewHistory ? statusBls.exitReviewTrip.value : statusBls.assignedTrip.value;
        const baseQuery = blsAggregateAssignTrip;
    ...
    return (
        <div className="app-main">
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-sm-6">
                            <h3 className="mb-0">Revisión de salida de viaje</h3>
                        </div>
                                    ...
                                    <td className='text-center'>
                                        {viewHistory ? (
                                            ...
                                            <button
                                                ...
                                                data-bs-target="#blsTripExitReviewModal"
                                                onClick={() => setBl(bl)}
                                                title="Revisar salida"
                                                disabled={Object.values(statusBls).find(s => s.value === bl.status)?.value !== statusBls.assignedTrip.value}
                                            >
            ...
            <TripExitReviewUnit bls={bl} refresh={() => { dispatch(getEntities(0, 20, searchQuery || undefined)); }} />
            {showPdfReport && <TripExitPdfReport bl={bl} onClose={() => setShowPdfReport(false)} />}
        </div>
    );
};

export default TripExitReview;
