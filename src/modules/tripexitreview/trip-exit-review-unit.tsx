...
type TripExitReviewUnitProps = {
  bls: IBLS | null;
  refresh: () => void;
};

interface ITripExitReviewUnitForm {
  frontal?: string;
...
const TripExitReviewUnit: React.FC<TripExitReviewUnitProps> = ({ bls, refresh }) => {
  ...
  const {
    ...
    reset,
    clearErrors,
  } = useForm<ITripExitReviewUnitForm>({
    mode: 'onChange',
  });
  ...
  const onSubmit = (data: ITripExitReviewUnitForm) => {
    if (bls) {
      const entity = {
        ...bls,
        ...data,
  ...
  return (
    <GenericModal id="blsTripExitReviewModal" title="Revisión de salida de viaje" size='lg'>
...
export default TripExitReviewUnit;
