...
type PortExitReviewUnitProps = {
  bls: IBLS | null;
  refresh: () => void;
};

interface IPortExitReviewUnitForm {
  frontal?: string;
...
const PortExitReviewUnit: React.FC<PortExitReviewUnitProps> = ({ bls, refresh }) => {
  ...
  const {
    ...
    reset,
    clearErrors,
  } = useForm<IPortExitReviewUnitForm>({
    mode: 'onChange',
  });
  ...
  const onSubmit = (data: IPortExitReviewUnitForm) => {
    if (bls) {
      const entity = {
        ...bls,
        ...data,
        status: statusBls.exitReviewPort.value,
        exitPhotos: [frontal, rightSide, leftSide, rear]
      };
  ...
  return (
    <GenericModal id="blsPortExitReviewModal" title="Revisión de salida de puerto" size='lg'>
...
export default PortExitReviewUnit;
