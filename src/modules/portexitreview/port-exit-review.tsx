...
import PortExitReviewUnit from './port-exit-review-unit';
import PortExitPdfReport from './port-exit-pdf-report';

const PortExitReview: React.FC = () => {
    ...
    const [searchQuery, setSearchQuery] = useState<string | null>(
        JSON.stringify([
            ...
            { $match: { alive: true, status: statusBls.assignedPort.value } },
        ])
    );
    ...
    const handleSearch = (query: string | null) => {
        ...
        const status = viewHistory ? statusBls.exitReviewPort.value : statusBls.assignedPort.value;
    ...
    const handleToggleView = () => {
        ...
        const status = newViewHistory ? statusBls.exitReviewPort.value : statusBls.assignedPort.value;
        const baseQuery = blsAggregateAssignPort;
    ...
    return (
        <div className="app-main">
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-sm-6">
                            <h3 className="mb-0">Revisión de salida de puerto</h3>
                        </div>
                                    ...
                                    <td className='text-center'>
                                        {viewHistory ? (
                                            ...
                                            <button
                                                ...
                                                data-bs-target="#blsPortExitReviewModal"
                                                onClick={() => setBl(bl)}
                                                title="Revisar salida"
                                                disabled={Object.values(statusBls).find(s => s.value === bl.status)?.value !== statusBls.assignedPort.value}
                                            >
            ...
            <PortExitReviewUnit bls={bl} refresh={() => { dispatch(getEntities(0, 20, searchQuery || undefined)); }} />
            {showPdfReport && <PortExitPdfReport bl={bl} onClose={() => setShowPdfReport(false)} />}
        </div>
    );
};

export default PortExitReview;
